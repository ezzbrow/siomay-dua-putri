<?= $this->include('partials/header') ?>

<main class="wizard-main">
    <?= $this->include('partials/progress', ['current' => 4]) ?>

    <div class="wizard-heading">
        <h1 class="wizard-title">
            <span class="material-symbols-outlined" aria-hidden="true">event</span>
            Pilih Tanggal Pesanan
        </h1>
        <p class="wizard-tagline">Pre-order minimal H+1. Sistem tidak menerima pesanan untuk hari yang sama.</p>
    </div>

    <?php if (session()->getFlashdata('error')): ?>
        <div class="wizard-flash wizard-flash-err" role="alert">
            <span class="material-symbols-outlined" aria-hidden="true">error</span>
            <span><?= esc(session()->getFlashdata('error')) ?></span>
        </div>
    <?php endif; ?>

    <form action="<?= base_url('checkout/tanggal') ?>" method="post" class="wizard-form">
        <?= csrf_field() ?>

        <div class="wizard-card">
            <div class="wizard-field">
                <label for="tanggal_dibutuhkan">Tanggal dibutuhkan <span class="required-mark">*</span></label>
                <input type="date" id="tanggal_dibutuhkan" name="tanggal_dibutuhkan"
                       min="<?= esc($besok) ?>"
                       value="<?= esc($tanggalValue) ?>" required>
                <p class="wizard-hint">
                    Pilih tanggal paling cepat besok. Pesanan diantar/diambil di tanggal ini.
                </p>
            </div>
        </div>

        <div class="wizard-actions">
            <a href="<?= base_url('checkout/catatan') ?>" class="wizard-btn wizard-btn-secondary">
                <span class="material-symbols-outlined" aria-hidden="true">arrow_back</span>
                Kembali
            </a>
            <button type="submit" class="wizard-btn wizard-btn-primary">
                Lanjut
                <span class="material-symbols-outlined" aria-hidden="true">arrow_forward</span>
            </button>
        </div>
    </form>
</main>

<style>
    .wizard-main { max-width: 640px; margin: 0 auto; padding: 32px 20px 0; }
    .wizard-heading { text-align: center; margin-bottom: 24px; }
    .wizard-title {
        font-size: 1.5rem; font-weight: 800; color: var(--on-surface);
        margin: 0 0 8px; display: inline-flex; align-items: center; gap: 10px;
    }
    .wizard-title .material-symbols-outlined { font-size: 28px; color: var(--primary); }
    .wizard-tagline { color: var(--on-surface-variant); margin: 0; font-size: 0.95rem; }
    .wizard-flash {
        padding: 12px 16px; border-radius: var(--radius-md); margin-bottom: 16px;
        display: flex; align-items: center; gap: 8px; font-weight: 500; font-size: 0.9rem;
    }
    .wizard-flash-err { background: var(--err-bg); color: var(--err-fg); border: 1px solid rgba(153, 27, 27, 0.18); }
    .wizard-flash .material-symbols-outlined { font-size: 20px; }
    .wizard-card {
        background: var(--surface); border: 1px solid var(--outline-variant);
        border-radius: var(--radius-lg); box-shadow: var(--card-shadow); padding: 24px;
    }
    .wizard-field label {
        display: block; font-weight: 600; font-size: 0.9rem;
        color: var(--on-surface); margin-bottom: 6px;
    }
    .required-mark { color: var(--accent); }
    .wizard-field input[type="date"] {
        width: 100%; padding: 12px 14px;
        border: 1.5px solid var(--outline-variant);
        border-radius: var(--radius-md);
        font-family: inherit; background: var(--surface);
        color: var(--on-surface); font-size: 1rem;
        min-height: 48px;
        transition: border-color var(--t-fast), box-shadow var(--t-fast);
    }
    .wizard-field input[type="date"]:focus {
        outline: none; border-color: var(--primary);
        box-shadow: 0 0 0 3px rgba(76, 29, 149, 0.18);
    }
    .wizard-hint { font-size: 0.85rem; color: var(--on-surface-variant); margin: 8px 0 0; }
    .wizard-actions {
        display: flex; justify-content: space-between; align-items: center;
        margin-top: 20px; gap: 12px;
    }
    .wizard-btn {
        display: inline-flex; align-items: center; gap: 6px;
        padding: 11px 20px; border-radius: var(--radius-md);
        font-size: 0.95rem; font-weight: 600; text-decoration: none;
        min-height: 44px; cursor: pointer; border: none;
        transition: background var(--t-fast), transform var(--t-fast);
    }
    .wizard-btn-primary {
        background: var(--primary); color: #fff;
        box-shadow: 0 4px 12px rgba(76, 29, 149, 0.20);
    }
    .wizard-btn-primary:hover { background: var(--primary-hover); transform: translateY(-1px); }
    .wizard-btn-secondary {
        background: var(--surface); color: var(--on-surface);
        border: 1.5px solid var(--outline-variant);
    }
    .wizard-btn-secondary:hover { background: var(--surface-variant); border-color: var(--secondary-light); }
    .wizard-btn .material-symbols-outlined { font-size: 18px; }
</style>

<?= $this->include('partials/footer') ?>
